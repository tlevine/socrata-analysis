...
This also pointed out to me that I should adjust my query above.
...
Then I ran queries like this to find what portals and data they used.
...
Using my most recent rule for selecting citizens from the user table,
I only see ``nrow(citizens)`` citizens. The few that I discussed above
have dozens of views, but most people have 1. The histogram below shows
how many views users have created. To make it easier to read, I left out
the top five users; they each have more than 20 views.

```{r citizen.views}
hist(citizens$n_views[citizens$n_views < 10],
  col = 1, border = NA,
...
<!-- discussion -->
That's a bit worrysome to me. I hope I just made a mistake or missed something,
but if I didn't, this means that only ``nrow(citizens)`` people are using Socrata
to create their own views of open data, that most of those people only ever create
one view, and that citizens have created only ``sum(citizens$n_views)`` views in
total. Keep in mind that these figures are totals across 

It could be that people are using these datasets a lot but that they're just
downloading spreadsheets and analyzing them in Excel (or R, as I'm doing),
and that's great. But that's the less interesting part about the Socrata
Open Data Portal software. Socrata is trying to "consumerize" data analysis,
to empower citizens to conduct their own analysis rather than relying on
external skilled data scientists.
