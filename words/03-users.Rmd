...
Most users (`r sum(users$n_views == 1)` to be exact) have exactly one view.
...
with `r users['e8ug-wzay','n_views']`. 
...
`r sum(!is.na(users$flags))` users have a non-empty `flags` field, and
...
But look at the absolute counts. Only `r sum(users$n_views >= 10)`
...
`r length(unique(socrata$portal))` portals; this number seems quite low.
...
to `r sum(is.na(users$flags) & is.na(users$roleName) & users$n_tables == 0 & users$n_views >= 10)`,
