...
```{r profile.image}
table('' == users$profileImageUrlLarge)
```

```{r missingness}
a <- t(as.data.frame(lapply(users, function(variable){ sum(nchar(variable) > 0) } )))
missingness <- data.frame(
  variable = rownames(a),
  present.value = a[,1]
)
barplot(
  log10(missingness$present.value),
  names.arg = missingness$variable,
  border = NA, col = 'grey', axes = F, horiz = T, las = 1,
  xlim = c(0, ceiling(log10(nrow(users))))
)
