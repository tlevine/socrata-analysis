...
Let's try that again, but I'm going to color the plot by whether the user is
an administrator, designer, editor, publisher or viewer.

```{r n.views.byrole}
plot.count(users$n_views, 'views',
  col = as.numeric(factor(users$roleName, levels =
    c(NA, 'administrator', 'designer', 'editor', 'publisher', 'viewer'))))
```
...
```{r n.tables}
plot.count(users$n_tables, 'tables', col = (users$id == 'e8ug-wzay') + 1)
...
```{r n.derived}
plot.count(users$n_views - users$n_tables, 'derived views', col = (users$id == 'e8ug-wzay') + 1)
```
