...
user.by.role <- ddply(users, 'roleName', function(users.role) {
  c(
    ...
    mean.number.of.tables = mean(users.role$n_tables),
    number.of.users = nrow(users.role)
  )
})
user.by.role[is.na(user.by.role$roleName),'roleName'] <- 'No role name'
ggplot(user.by.role) +
  aes(x = mean.number.of.views, y = mean.number.of.tables) +
  geom_point(aes(size = number.of.users)) +
  geom_text(aes(label = roleName), hjust = -0.2) +
  scale_size_continuous('Number of users', labels = comma) +
  scale_x_continuous('Mean number of views each user owns', limits = c(0, 100)) +
  scale_y_continuous('Mean number of tables each user owns') +
