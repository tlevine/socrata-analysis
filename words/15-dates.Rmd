...
2. It is counting the number of [tables]()
    rather than the number of [views]().
...
a new version. For example, XXX uploaded 2010 data
[here]()
and 2011 data
[here]().
...
these statistics, I'm using the total across all [views]() in the
[table/family](), not just the value for that particular view.
...
Note well: These links go to arbitrary [views](/!/) on the particular
[table]() that was updated; you'll have to follow a few links to get to
...
the file doesn't always come out on time, so we might only recieve
...
I suppose this says something about the need for more measures of the progress of
