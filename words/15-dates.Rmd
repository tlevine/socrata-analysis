...
`r length(unique(socrata.deduplicated$portal))`
data portals run by [Socrata](http://socrata.com).

### Removing duplicates
I had to do a lot to remove duplicate datasets.
I group the causes of duplication into two categories:
federation and derived views.

Dealing with federation was quite straightforward;
...
Only `r sum(socrata.deduplicated$has.been.updated)` of the
`r nrow(socrata.deduplicated)` datasets have ever been updated.
...
data-driven decisions about our open data efforts. For describing our current
situation, I quite like the phrase "You get what you measure." (or any of its
variants). We say a lot of things about how people should be doing open data,
...
### Current metrics
Well actually we sort of do at least count the number of datasets.

Socrata has some relevant metrics built-in.
The main metric is the number of datasets.
That number is much less meaningful than you might think, as Andrew Nicklin
[explains](http://technickle.nicklin.info/post/58169276432/accurately-counting-socrata-datasets),
but it still tells us something.

The [site-wide usage metrics](/!/socrata-metrics-api/) tell us some other things, though
they're probably really intended for diagnosing technical issues with the servers.

`next.data.gov` has ever-so-slightly advanced its [metrics page](http://next.data.gov/metric)
by adding a "Latest Entry" number (I don't know what that means.) and breaking
out the statistics by agency.

Even with all of these issues, I think the dataset count metric is at least
somewhat meaningful and helpful. We know that we should be increasing that
number, and we can make vague comparisons that give us some idea as to whether
we are progressing and how one portal/agency stands in relation to another.
We are measuring dataset counts, and dataset counts are increasing.

### Other metrics
Getting more datasets is great, but there are a bunch of other things that
we want.
