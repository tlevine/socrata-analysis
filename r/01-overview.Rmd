...
```
.downloadCount <- ddply(socrata, 'portal', function(df) { c(downloadCount = sum(df$downloadCount)) })
socrata$portal <- factor(socrata$portal,
  levels = .downloadCount$portal[order(.downloadCount$downloadCount, descreasing = T)])

ggplot(.downloadCount) + aes(x = portal, y = downloadCount) + geom_point() + coord_flip() +
...
Average dataset size by portal (ncol, nrow, ncol * nrow)
...
When do datasets get uploaded? Does it happen in bursts?
