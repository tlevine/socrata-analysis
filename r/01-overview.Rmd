...
<style>article img { width: 100%; margin-left: 0; margin-right: 0; }</style>
```{r configure, echo = F}
opts_chunk$set(fig.width=10, fig.height=6, dip = 42 * 3)
```
...
set.seed(13984382)
print(listify(sample.int(nrow(deleted), 10)))
```
{:.language-r}
...
```{r dataset_size_by_portal_ncol}
base + aes(y = ncol)
```
{:.language-r}

```{r dataset_size_by_portal_ncell}
...
```{r datasets_size_over_time_nrow}
...
```{r datasets_size_over_time_ncol}
base + aes(y = ncol)
```
{:.language-r}

```{r datasets_size_over_time_ncell}
base + aes(y = nrow * ncol)
```
{:.language-r}
